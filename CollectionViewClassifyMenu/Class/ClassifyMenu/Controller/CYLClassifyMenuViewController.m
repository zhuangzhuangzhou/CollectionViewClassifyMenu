...
- (void)judgeMoreButtonShowWhenDefaultRowsCount:(NSUInteger)defaultRowsCount {
    [self.rowsCountPerSection enumerateObjectsUsingBlock:^(id  __nonnull obj, NSUInteger idx, BOOL * __nonnull stop) {
        ...
        if ([obj integerValue] > defaultRowsCount) {
            ...
            //NSLog(@"大-------%@",self.collectionHeaderMoreBtnHideBoolArray);
        } else {
            [self.collectionHeaderMoreBtnHideBoolArray replaceObjectAtIndex:idx withObject:@YES];
            //NSLog(@"小-------%@",self.collectionHeaderMoreBtnHideBoolArray);
        }
    }];
...
- (UIView *)addTableHeaderView {
    ...
    //仅修改titleLabel的宽度,xyh值不变 [为什么要修改]
...
//【每组】都分两种情况：点开“更多”按钮和没点开。 1.没点，返回该组第一行的个数。2.某一行或者几行点开，则self.expandSectionArray有值（其值为indexPath.section ：filterHeaderView.moreButton.tag = indexPath.section），然后通过循环比较是否存在该组indexPath.section 来进行item个数的返回
...
- (UICollectionReusableView *)collectionView:(UICollectionView *)collectionView
    ...
    if ([kind isEqual:UICollectionElementKindSectionHeader]) {
        ...
        [self.collectionHeaderMoreBtnHideBoolArray[indexPath.section] boolValue];// "更多按钮的是否隐藏通过这个数组来进行判断"
